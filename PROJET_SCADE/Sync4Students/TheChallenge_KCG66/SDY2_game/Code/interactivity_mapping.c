...
void SDY2_game_locate(SGLlong pPointer, SGLfloat* pX, SGLfloat* pY) {
  int index;
...
void SDY2_game_pointer(SGLlong pPointer, SGLbool* pPressed, SGLbool* pReleased, SGLlong* pButton, SGLfloat* pX, SGLfloat* pY, SGLlong* pModifiers) {
  int index;
  for (index=0; index<last_pointer_event; index++) {
    const sdy_pointer_event_t *lPointerEvent = &pointer_events[index];
    if ((pPointer == -1) || (lPointerEvent->id == -1) || (pPointer == lPointerEvent->id)) {
...
void SDY2_game_keyboard(SGLlong pKeyboard, SGLbool* pPressed, SGLbool* pReleased, SGLlong* pCode, SGLlong* pModifiers) {
  int index;
...
void SDY2_game_cursor(SGLlong pCursor, SGLfloat pX, SGLfloat pY) {
